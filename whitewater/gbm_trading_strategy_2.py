...
class WhitewaterSpotStrategy:
    ...
    def load_and_prep(self):
        ...
        df = price_df.join([weather_pivoted, lng_df], how='inner')
        ...
        # --- TOGGLE TEST START ---
        # Option A: The "Perfect" Look-ahead (Original)
        df['tomorrow_min_temp_MAF'] = df['min_temp_f_MAF'].shift(-1)

        # Option B: The "Blended" Forecast (Today + Tomorrow / 2)
        df['tomorrow_min_temp_MAF'] = (df['min_temp_f_MAF'] + df['min_temp_f_MAF'].shift(-1)) / 2
        # --- TOGGLE TEST END ---

        df['tomorrow_min_temp_MAF'] = df['min_temp_f_MAF']

        self.full_df = df.drop(columns=['Agua Dulce', 'min_temp_f_PEQ'], errors='ignore').dropna().sort_index()
        return self
    ...
    def simulate_spot_logic(self):
        """Applies Physical Spot TC: pay on full position daily."""
        df = self.results_df
        ...
        for s in self.spreads:
            ...
            # SPOT TC: No turnover penalty, just pay on held volume
    ...
    def export(self):
        ...
        print(f"Strategy simulation complete. Reference: whitewater_current.csv")
...
if __name__ == "__main__":
    ...
    base_lots = 15
    high_vol_lots = 50
    strategy = WhitewaterSpotStrategy(XLSX, high_vol_lots=high_vol_lots, base_lots=base_lots)
